...
export async function ReviewQueue() {
  ...
  const [{ data: newPosts }, { data: editPosts }] = await Promise.all([
    // New posts awaiting first-time approval
    supabase
      .from('posts')
      .select(`
        *,
    ...
    // Approved posts with a pending edit submitted for review
    supabase
      .from('posts')
      .select(`
        *,
  ...
  if (isEmpty) {
    return (
      <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 py-16 text-center">
        <p className="text-sm text-gray-500">No posts pending review.</p>
      </div>
    )
  }
        ...
        <div key={post.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <Link
              ...
              className="block truncate text-base font-medium text-gray-900 hover:text-saffron-700"
            >
